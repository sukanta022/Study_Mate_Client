...
const MyConnections = () => {
    ...
    useEffect(() => {
        if(user && usersData){
            ...
            if(accountUser?.partner){
                const accountUserPartner = usersData.filter(users => accountUser.partner.includes(users.email))
                setAllPartner(accountUserPartner)
            }  
        }
    },[user,usersData])
        ...
        <div className='bg-[#0F172A] min-h-screen flex flex-col items-center gap-5 p-5'>
            ...
            <div className="overflow-x-auto w-full">
                <table className="min-w-full text-white border-collapse border border-[#262D62]">
                    ...
                    {/* Table Body */}
                    <tbody>
                    {allPartner.map((userPartner, idx) => (
                        ...
                        {/* Action */}
                        <td className="py-4 px-4">
                            <button onClick={() => handleDeletePartner(userPartner._id, userPartner.email)} className="btn btn-sm bg-red-500 hover:bg-red-600 text-white flex items-center gap-1">
